...
const AddAction = ({ membersData }: { membersData?: Member[] }) => {
  // States
  const [open, setOpen] = useState(false)
  const [count, setCount] = useState(1)
  const [selectData, setSelectData] = useState<Member | null>(null)
  ...
  const [invoiceNumber, setInvoiceNumber] = useState<string>(`INV${Date.now().toString().slice(-8)}`)
  ...
  const onMemberCreated = (memberId: string, memberName: string) => {
    ...
    // Add to members list and auto-select
    const updatedMembers = [newMember, ...members]
    setMembers(updatedMembers)
    setSelectData(newMember)
  ...
  return (
    <>
      <AddCustomerDrawer open={open} setOpen={setOpen} onFormSubmit={onFormSubmit} onMemberCreated={onMemberCreated} />
      <Card>
              ...
              <div className='flex justify-between flex-col gap-4 flex-wrap sm:flex-row'>
                <div className='flex flex-col gap-4'>
                  ...
                  <CustomTextField
                    ...
                    onChange={e => {
                      setFormData({} as FormDataType)
                      setSelectData(members?.filter(item => item.id === e.target.value)[0] || null)
                    }}
                  >
                    <MenuItem
                    ...
                    {members?.slice(0, 10).map((member: Member, index) => (
                  ...
                  {selectData?.id ? (
                    <div>
                      <Typography>{selectData?.firstName} {selectData?.lastName}</Typography>
