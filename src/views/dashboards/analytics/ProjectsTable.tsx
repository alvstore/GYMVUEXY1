...
// Type Imports
import type { GymMemberTableRowType } from '@/types/pages/profileTypes'
...
// Column Definitions
const columnHelper = createColumnHelper<GymMemberTableRowType>()

const ProjectTables = ({ projectTable }: { projectTable?: GymMemberTableRowType[] }) => {
  // States
  ...
  // Hooks
  const columns = useMemo<ColumnDef<GymMemberTableRowType, any>[]>(
    () => [
      {
        id: 'select',
        header: ({ table }) => (
      ...
      columnHelper.accessor('leader', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            <CustomAvatar src={`/images/avatars/trainer-${row.original.id}.png`} size={34} />
            <Typography color='text.primary'>{row.original.leader}</Typography>
          </div>
        )
      }),
      columnHelper.accessor('avatarGroup', {
        header: 'Membership Elapse',
        cell: ({ row }) => {
          const daysElapsed = row.original.status || 0
  ...
  const table = useReactTable({
    data: data as GymMemberTableRowType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
